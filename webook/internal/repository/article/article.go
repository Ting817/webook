...
func NewArticleRepository(dao dao.ArticleDAO, c cache.ArticleCache, userRepo repository.UserRepository, l logger.LoggerV1) ArticleRepository {
...
func (repo *CachedArticleRepository) List(ctx context.Context, uid int64, offset int, limit int) ([]domain.Article, error) {
	...
	if offset == 0 && limit == 100 {
		...
		if err != cache.ErrKeyNotExist {
			repo.l.Error("查询缓存文章失败", logger.Int64("author", uid), logger.Error(err))
		}
	}
	...
	if err != nil {
		repo.l.Error("刷新第一页文章的缓存失败", logger.Int64("author", uid), logger.Error(err))
	}
	return res, nil
}
